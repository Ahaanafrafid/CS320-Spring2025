...
namespace MyCookBookApi.Repositories
{
    public class MockRecipeRepository : IRecipeRepository
    {
        ...
        {
            if (recipe == null || string.IsNullOrWhiteSpace(recipe.Name) || recipe.Ingredients == null || !recipe.Ingredients.Any())
                throw new ArgumentException("Invalid recipe data");

            recipe.RecipeId = Guid.NewGuid().ToString();  // Ensure each recipe has a unique ID
            _recipes.Add(recipe);
        }
        ...
        public bool UpdateRecipe(string id, Recipe updatedRecipe)
        {
            ...
            updatedRecipe.RecipeId = id;  // Preserve the original ID
